...
<c:set var="isAssignment" value="${not empty selectedAssessment and selectedAssessment.type == 'Assignment'}" />
    ...
    <div class="content-wrapper">
        ...
        <section class="ins-page-head">
            <div>
                ...
                <h2><c:out value="${isAssignment ? 'Assignment Items for ' : 'Bank for '}"/>${selectedAssessment.title}</h2>
            </div>
            <div class="ins-hero-actions">
                ...
                <a href="${pageContext.request.contextPath}/instructor/assessments?view=dashboard&courseId=${selectedCourse.courseId}&assessmentId=${selectedAssessment.assessmentId}&success=draftsaved" class="btn btn-secondary">
                    <i class="fas fa-floppy-disk"></i> Save Draft
                </a>
                <a href="${pageContext.request.contextPath}/instructor/assessments?view=dashboard&courseId=${selectedCourse.courseId}&assessmentId=${selectedAssessment.assessmentId}&success=finished" class="btn btn-secondary">
                ...
                <form method="post" action="${pageContext.request.contextPath}/instructor/assessments" class="ia-inline-form">
                    ...
                    <button type="submit" class="btn btn-primary" ${empty questions ? 'disabled' : ''} title="${empty questions ? 'Add at least one question before publishing.' : 'Publish this assessment'}">
        ...
        <!-- Messages -->
        <c:if test="${param.success == 'created'}"><div class="alert alert-success"><i class="fas fa-check-circle"></i> Assessment created. Continue by adding questions, then save draft or publish.</div></c:if>
        <c:if test="${param.success == 'draftsaved'}"><div class="alert alert-success"><i class="fas fa-check-circle"></i> Draft saved. You can publish anytime when ready.</div></c:if>
        <c:if test="${param.success == 'finished'}"><div class="alert alert-success"><i class="fas fa-check-circle"></i> Assessment setup completed. You can continue editing anytime.</div></c:if>
        ...
        <div class="ia-flow-grid-question">
            ...
            <div class="section-card ia-flow-sticky" style="padding: 24px;">
                ...
                <form method="post" action="${pageContext.request.contextPath}/instructor/assessments" class="ia-form ia-question-form" enctype="multipart/form-data">
                    ...
                    <div class="ia-question-form-block">
                        <label for="createQuestionText" class="field-label"><c:out value="${isAssignment ? 'Assignment Prompt / Questions' : 'Question Prompt'}"/> <span style="color: #dc2626;">*</span></label>
                        <textarea id="createQuestionText" name="questionText" rows="4" placeholder="${isAssignment ? 'Enter the assignment instructions or question text...' : 'Enter the question text...'}" ${isAssignment ? '' : 'required'}></textarea>
                    </div>

                    <c:if test="${not isAssignment}">
                        <div class="ia-question-form-grid">
                            <div>
                                <label class="field-label">Option A <span style="color: #dc2626;">*</span></label>
                                <input name="optionA" type="text" placeholder="First option" required />
                            </div>
                            <div>
                                <label class="field-label">Option B <span style="color: #dc2626;">*</span></label>
                                <input name="optionB" type="text" placeholder="Second option" required />
                            </div>
                            <div>
                        ...
                        <div class="ia-question-form-split">
                            <div>
                                <label for="createCorrectOption" class="field-label">Answer <span style="color: #dc2626;">*</span></label>
                            ...
                            <div>
                                ...
                                <input id="createMarks" name="marks" type="number" min="0.5" step="0.5" value="1" required />
                            </div>
                        </div>
                    </c:if>
                    ...
                    <button class="btn btn-primary" type="submit">
                        <i class="fas fa-save"></i> <c:out value="${isAssignment ? 'Add Assignment Item' : 'Add to Bank'}"/>
                    </button>
                </form>
            ...
            <div>
                <div class="ia-title-row">
                    ...
                    <span class="ia-count-pill"><strong>${fn:length(questions)}</strong> Total Items</span>
                </div>
                                    ...
                                    <c:choose>
                                        <c:when test="${isAssignment}">
                                            <div style="display:grid; gap: 12px; margin-top: 8px;">
                                                <div class="ia-option-item" style="justify-content: space-between;">
                                                    <span><strong>Assignment prompt:</strong> ${q.questionText}</span>
                                                </div>
                                                <c:if test="${not empty q.attachmentUrl}">
                                                    <div class="ia-option-item" style="justify-content: space-between;">
                                                        <span><strong>Attachment:</strong> ${not empty q.attachmentName ? q.attachmentName : 'Open PDF'}</span>
                                                        <a class="btn btn-secondary btn-sm" href="${q.attachmentUrl}" target="_blank" rel="noopener noreferrer">
                                                            <i class="fas fa-up-right-from-square"></i> Open PDF
                                                        </a>
                                                    </div>
                                                </c:if>
                                            </div>
                                        ...
                                        <c:otherwise>
                                            <div class="ia-options-grid">
                                                <div class="ia-option-item ${q.correctOption == 'A' ? 'is-correct' : ''}">
                                                ...
                                                <div class="ia-option-item ${q.correctOption == 'B' ? 'is-correct' : ''}">
                                                ...
                                                <c:if test="${not empty q.optionC}">
                                                    <div class="ia-option-item ${q.correctOption == 'C' ? 'is-correct' : ''}">
                                                ...
                                                <c:if test="${not empty q.optionD}">
                                                    <div class="ia-option-item ${q.correctOption == 'D' ? 'is-correct' : ''}">
